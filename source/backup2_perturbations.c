...
int perturbations_init(
  ...
  if (pba->has_adm == _TRUE_){
    ...
    pth->b_idr = 0;
    pth->tau_idr_free_streaming=pth->tau_free_streaming;

    short *ba1 = &(pth->has_idm_dr);
    *ba1 = _TRUE_;
    short  *ba2 = &(pba->has_idm);
    *ba2 = _TRUE_;
...
int perturbations_derivs(double tau,
  ...
  if (pba->has_idm == _TRUE_) {
    ...
    if ((pth->has_idm_dr == _TRUE_)){
      ...
      S_idm_dr = 4./3. * pvecback[pba->index_bg_rho_adr]/ pvecback[pba->index_bg_rho_adm];
  ...
  if (_scalars_) {
	...
	if (ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_off) {
	  dy[pv->index_pt_theta_idm] += -S_idm_dr * dmu_idm_dr * (theta_idm - theta_idr); /* correction to idm velocity due to idm_dr when tca_idm_dr is off */
          ...
          tca_slip_idm_dr = (/*pth->n_index_idm_dr*/2.-2./(1.+S_idm_dr))*a_prime_over_a*( theta_idm - theta_idr)
