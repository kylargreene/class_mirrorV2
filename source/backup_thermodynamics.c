...
int thermodynamics_at_z(
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    //if (pba->has_adm == _FALSE_){
	    //printf("WARNING: natural IDM module is running!\n");
...
int thermodynamics_init(
  ...
  //small mirror edit; including adm flag in the pth struct as not all functions see pba
  if (pba->has_adm == _TRUE_) {
...
//START MIRROR EDIT
//utilizing the pre-made idm-idr perturbation modual
...
int thermodynamics_workspace_init(
  ...
  case hyrec:
    ...
    if (pba->has_adm == _TRUE_) {
      /*
      class_alloc(ptw->d_ptdw->dark_phyrec, sizeof(struct thermohyrec), pth->error_message);
      class_call(thermodynamics_hyrec_init(ppr,pba,pth,ptw->SIunit_dark_nH0,pba->T0_adr,0., ptw->d_ptdw->dark_ap_z_limits[ptw->d_ptdw->index_dark_ap_saha],ptw->d_ptdw->dark_phyrec),
...
int thermodynamics_indices(
  ...
  if (pba->has_adm == _TRUE_) {
    ...
    //class_define_index(pth->index_th_dmu_idm_dr,_TRUE_, index_th, 1);
...
int thermodynamics_solve(
  ...
  //printf("before dark free \n\n");
  if (ptw->d_ptdw->dark_ap_size != 0) {
    class_call(dark_thermodynamics_vector_free(ptw->d_ptdw->d_ptv),
               pth->error_message,
               pth->error_message);
  }
  //printf("after dark free \n\n");
...
int thermodynamics_calculate_remaining_quantities(
  ...
  if (pba->has_idr == _TRUE_ || pth->has_idm_b == _TRUE_) {
    //MIRROR NOTE: WE CAN LEAVE THIS UNTOUCHED HERE
